...
def push_prompts_to_hub(client: Client):
    ...
    """
    try:
        url = client.push_prompt(PROMPT_V1_NAME, object=PROMPT_V1, description="V1 – ngắn gọn, trực diện")
        print(f"✅ Đã push V1 → {url}")
    except Exception as e:
        print(f"⚠️  V1 lỗi: {e}")

    try:
        url = client.push_prompt(PROMPT_V2_NAME, object=PROMPT_V2, description="V2 – có cấu trúc, chuyên gia")
    ...
    for attempt in range(5):
        try:
            ...
            if "429" in str(e) or "rate" in str(e).lower():
                ...
                print(f"  ⏳ Rate limit gặp phải, đợi {wait_time}s thử lại...")
...
def main():
    ...
    client = Client(
        ...
        api_url=config.os.environ.get("LANGCHAIN_ENDPOINT"),
    )
